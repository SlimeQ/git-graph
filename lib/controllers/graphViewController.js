 ...
 * mounted.
 *
 * @param {Grapher} projectGrapher - a Grapher for the current project
 */
function toggleGraph(projectGrapher) {
  ...
  if (!editorView.graphView) {
    var remoteUrl = projectGrapher.repo.getOriginURL(filePath);
    var remoteRevision;
    try {
