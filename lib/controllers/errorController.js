...
const GraphErrorView = require('../views/graph-error-view');
const strings = require('../locales/strings');

/**
 * A mapping from git error message fragment to the strings error key. When an
 * error is thrown by the graphr, we try to match the errors message to something
...
function showError(errorMessageKey) {
  ...
  if (messageString) {
    var errorView = new GraphErrorView({message: messageString});
    errorView.attach();
  }
