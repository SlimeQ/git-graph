 ...
 * @module Grapher
 *
 * Grapher is a Class that should be instantiated with an atom 'Git' object
 ...
 */
var Grapher = function(repo) {
  if (!repo) {
    throw new Error('Cannot create a Grapher without a repository.');
  }
...
_.extend(Grapher.prototype, {

  /**
   * Initializes this Grapher instance, by creating git-tools repos for the root
  ...
  /**
   * Graphs the given filePath and calls callback with graph lines or error.
   *
   * @param {string} filePath - filePath to graph
   * @param {function} callback - callback to call back with graph data
   */
  graph: function(filePath, callback) {
    ...
    // Make the async graph call on the git repo
    repoUtil.graph(filePath, function(err, graph) {
      callback(err, graph);
    });
  },
